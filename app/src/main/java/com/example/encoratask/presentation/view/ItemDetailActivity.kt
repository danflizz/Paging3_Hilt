...
class ItemDetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            ...
            val id = intent.getStringExtra(ItemDetailFragment.ARG_ITEM_ID)
            id?.let {
                lifecycleScope.launchWhenStarted {
                    mainViewModel.fetchCharacterById(id).collect {
                        displayFragment(it)
                    }
                }
            }
    ...
    private fun displayFragment(item: DetailsResponse?) {
